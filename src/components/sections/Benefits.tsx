...
function BenefitCard({ benefit, index }: { benefit: Benefit; index: number }) {
  return (
    <motion.div
      ...
      className={`bg-bone rounded-sm p-6 border-t-4 ${accentMap[benefit.accentColor]} border border-straw/40 flex flex-col gap-3`}
    >
      <span className="text-3xl" aria-hidden="true">{benefit.icon}</span>
      <h3 className="font-display font-light text-lg text-dark-oak">{benefit.title}</h3>
      <p className="text-sm text-umber font-body leading-relaxed">{benefit.description}</p>
    </motion.div>
  )
}
...
export default function Benefits() {
  return (
    <section id="benefits" className="py-20 md:py-24 bg-raw-linen">
